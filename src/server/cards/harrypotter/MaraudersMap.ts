...
export class MaraudersMap extends Card implements IActionCard, IProjectCard {
  constructor() {super({type: CardType.ACTIVE, name: CardName.MARAUDERS_MAP, tags: [Tag.SPACE, Tag.SCIENCE], cost: 15, victoryPoints: 1, metadata: {cardNumber: 'HP96', renderData: CardRenderer.builder((b) => {b.action('Draw 1 card.', (ab) => {ab.empty().startAction.cards(1);});}), description: 'Action: Draw 1 card. (I solemnly swear that I am up to no good.)'}});}
