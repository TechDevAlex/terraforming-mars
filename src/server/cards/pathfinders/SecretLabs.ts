...
export class SecretLabs extends Card implements IProjectCard {
  ...
  public override bespokePlay(player: IPlayer) {
    ...
    if (player.canAfford({cost: 0, tr: {oceans: 1}})) {
      ...
      const optionTitle = oceanPlacementAvailable ? 'Place an ocean tile. Add 3 microbes to ANY card.' : 'Add 3 microbes to ANY card.';
    ...
    if (player.canAfford({cost: 0, tr: {temperature: 1}})) {
      options.options.push(new SelectOption('Raise temperature 1 step. Gain 4 plants.').andThen(() => {
    ...
    if (player.canAfford({cost: 0, tr: {oxygen: 1}})) {
      options.options.push(new SelectOption('Raise oxygen level 1 step. Add 3 floaters to ANY card.').andThen(() => {
