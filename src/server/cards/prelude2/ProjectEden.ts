...
export class ProjectEden extends PreludeCard {
    ...
    super({
      ...
      metadata: {
        ...
        renderData: CardRenderer.builder((b) => {
          b.oceans(1).city().greenery().text('-4').cards(1);
        }),
        description: 'Place 1 ocean tile, 1 city tile, and 1 greenery tile. Discard 4 cards.',
      },
    });
  }

  public override bespokeCanPlay(player: IPlayer): boolean {
    if (player.cardsInHand.length >= 4 && player.canAfford({cost: 0, tr: {oceans: 1, oxygen: 1}})) {
  ...
  private selectNextAction(player: IPlayer): void {
    ...
    if (!this.selected.includes('discard')) {
      options.push(
        new SelectOption('Discard 4 cards').andThen(() => {
          this.selected.push('discard');
          player.game
            .defer(new DiscardCards(player, 4, 4, 'Select 4 cards to discard'))
            .andThen(() => this.selectNextAction(player));
          return undefined;
        }),
      );
